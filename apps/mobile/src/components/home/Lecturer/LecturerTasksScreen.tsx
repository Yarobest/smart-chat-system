...
export default function LecturerTasksScreen() {
  ...
  const [assignments,setAssignments]=useState<Assignment[]>([]); const [quizzes,setQuizzes]=useState<Quiz[]>([]); const [materials,setMaterials]=useState<CourseMaterial[]>([]); const [loading,setLoading]=useState(true);
  useFocusEffect(useCallback(()=>{let live=true;Promise.all([assignmentService.list(),quizService.list(),materialService.list()]).then(([a,q,m])=>{if(live){setAssignments(a);setQuizzes(q);setMaterials(m)}}).finally(()=>live&&setLoading(false));return()=>{live=false}},[]));
