...
export default function StudentAssignmentDetailScreen() {
  ...
  const load = useCallback(() => {
    ...
    assignmentService
      ...
      .then(setAssignment)
      .catch((error) =>
        Alert.alert("Could not load assignment", error.message),
      );
  }, [id]);
  useEffect(() => {
    load();
  ...
  const submit = async () => {
    ...
    const hasResponse =
      (assignment.allowText && Boolean(response.trim())) ||
      (assignment.allowFile && files.length > 0);
  ...
  if (!assignment)
    return (
      <SafeAreaView className="flex-1 bg-[#F5F7FA]">
        <PageLoader label="Loading assignment..." />
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#051839]">
      ...
      <ScreenHeader
        ...
        fallbackRoute="/(student)/tasks/assignments"
      />
      <KeyboardAwareView>
        <ScrollView
        ...
        >
          <View className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <Text className="mt-1 text-sm font-semibold text-slate-500">
              Total marks available: {assignment.totalMarks}
            </Text>
          ...
          {assignment.submission ? (
            <View className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
              ...
              <Text className="mt-1 text-sm text-emerald-700">
                Version {assignment.submission.version} ·{" "}
                {new Date(assignment.submission.submittedAt).toLocaleString()}
              </Text>
              {assignment.recordedScore !== null &&
              assignment.recordedScore !== undefined ? (
                <View className="mt-3">
                  ...
                  <Text className="mt-1 text-lg font-extrabold text-emerald-900">
                    {assignment.recordedScore} out of {assignment.totalMarks}
                  </Text>
                  {assignment.recordedGradeVersion ? (
                    <Text className="mt-1 text-xs text-emerald-700">
                      Highest released grade · Version{" "}
                      {assignment.recordedGradeVersion}
                    </Text>
                  ) : null}
                </View>
              ) : (
                <Text className="mt-3 text-sm font-semibold text-emerald-700">
                  Awaiting lecturer grading
                </Text>
              )}
              {assignment.recordedFeedback ? (
                <Text className="mt-2 text-sm text-emerald-800">
                  Feedback: {assignment.recordedFeedback}
                </Text>
              ) : null}
            </View>
          ) : null}
          ...
          {submissionOpen ? (
            <View className="rounded-2xl border border-slate-200 bg-white p-4">
              <Text className="mb-3 font-extrabold text-slate-900">
                {assignment.submission
              ...
              {assignment.allowText ? (
                <TextInput
                  ...
                  className="min-h-32 rounded-xl border border-slate-200 px-3 py-3 text-sm"
                />
              ) : null}
              ...
              {!hasResponse ? (
                <Text className="mt-3 text-center text-xs font-semibold text-slate-400">
                  {assignment.allowFile && assignment.allowText
                    ? "Upload a file or write a response to enable submission."
                    : assignment.allowFile
                      ? "Upload a file to enable submission."
                      : "Write a response to enable submission."}
                </Text>
              ) : null}
              <Pressable
                disabled={submitDisabled}
                onPress={submit}
                className={`mt-4 items-center rounded-full py-3 ${submitDisabled ? "bg-slate-300" : "bg-blue-600"}`}
              >
                <Text
                  className={`font-bold ${submitDisabled ? "text-slate-500" : "text-white"}`}
                >
