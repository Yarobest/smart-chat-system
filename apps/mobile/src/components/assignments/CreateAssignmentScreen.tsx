...
export default function CreateAssignmentScreen() {
  ...
  useEffect(() => {
    const offeringsRequest = assignmentService
      .courseOfferings()
      .then(setOfferings);
  ...
  const save = async (publish: boolean) => {
    if (!courseOfferingId || !title.trim() || !instructions.trim()) {
      Alert.alert(
        "Missing information",
        "Select a course and enter a title and instructions.",
      );
      return;
    }
    try {
      ...
      Alert.alert(heading, message, [
        {
          ...
          onPress: () =>
            router.replace(
              (assignmentId
                ? `/(lecturer)/courses/assignments/${assignmentId}`
  ...
  const selectedCourse = offerings.find(
    (offering) => offering.id === courseOfferingId,
  );
      ...
      <KeyboardAwareView>
        ...
        >
          <View>
            ...
            <Pressable
              onPress={() => {
                if (!isEditing) setCourseDropdownOpen((current) => !current);
              }}
                ...
                <ScrollView nestedScrollEnabled>
                  {offerings.map((offering) => (
                    ...
                    >
                      <View className="flex-1">
                        <Text
                          className={`text-sm font-bold ${courseOfferingId === offering.id ? "text-blue-700" : "text-slate-800"}`}
                        >
                          {offering.courseCode} · {offering.courseName}
                        </Text>
                        <Text className="mt-1 text-xs text-slate-500">
                          {[offering.programme, offering.yearGroup]
                            .filter(Boolean)
                            .join(" · ")}
                        </Text>
                      </View>
          ...
          <View>
            ...
            <View className="flex-row gap-3">
              <Pressable
                ...
                className="flex-1 flex-row items-center rounded-2xl border border-slate-200 bg-white px-4 py-3"
              >
              ...
              <View className="mt-2 rounded-2xl border border-slate-200 bg-white p-2">
                ...
                {Platform.OS === "ios" ? (
                  <Pressable
                    ...
                    className="self-end rounded-full bg-blue-600 px-5 py-2"
                  >
          ...
          <View>
            ...
            {existingSubmissionCount > 0 ? (
              <Text className="mt-2 text-xs font-semibold text-slate-500">
                Total marks are locked because students have already submitted.
                You can still edit the instructions, deadline, late submissions,
                and resubmission settings.
              </Text>
            ) : null}
          </View>
          ...
          {isEditing ? (
            ...
            <View className="flex-row gap-3">
              <Pressable
                disabled={saving}
                onPress={() => save(false)}
                className="flex-1 items-center rounded-full border border-blue-600 bg-white py-3"
              >
