...
export default function UserManagementScreen() {
  ...
  const lecturersCount = users.filter(
      ...
      <View className="flex-1 bg-[#F3F6FD]">
        <AdminPageHeader
          ...
          subtitle={
            loading
              ? "Loading live users..."
              : `${total} users · ${onlineCount} online`
          }
        />
        ...
        >
            ...
            >
              {filters.map((filter, index) => {
                ...
                return (
                  ...
                  >
                    <Text
                      className={`text-sm font-bold ${active ? "text-white" : "text-slate-600"}`}
                    >
                      {filter}
                    </Text>
                  </Pressable>
          ...
          <View className="bg-white">
            {filteredUsers.map((user) => (
              <View
                key={user.id}
                className="mx-4 mb-3 flex-row items-center rounded-2xl border border-slate-100 bg-white p-4 shadow-sm shadow-slate-200"
              >
                <View className="flex-1 flex-row items-center">
                  <View className="mr-3 h-12 w-12 items-center justify-center rounded-full bg-[#3B6AE3]">
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="mt-1 text-sm text-slate-400">
                      {user.studentId ?? user.staffId ?? user.email} ·{" "}
                      {user.role} · {user.department ?? "No department"}
                    </Text>

                    <View className="mt-1 flex-row items-center">
                      <View
                        className={`mr-2 h-4 w-4 rounded-full ${user.isOnline ? "bg-lime-500" : "bg-slate-300"}`}
                      />
                ...
                <Pressable
                  onPress={() =>
                    router.push(`/(admin)/users/${user.id}` as never)
                  }
                  className="ml-3 rounded-lg bg-blue-50 px-3 py-2 active:bg-blue-100"
                >
                  <Text className="text-sm font-bold text-blue-700">View</Text>
                </Pressable>
              </View>
            ))}
