...
export default function LecturerMaterialsList() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#051839]">
      ...
      <ScreenHeader title="Notes & Slides" fallbackRoute="/(lecturer)/tasks" />
      <ScrollView
        className="flex-1 bg-[#F5F7FA]"
        contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 30 }}
      >
        <Pressable
          onPress={() => router.push("/(lecturer)/courses/push-note" as any)}
          className="items-center rounded-lg bg-blue-600 py-3"
        >
          <Text className="font-bold text-white">+ Upload Material</Text>
        </Pressable>
        ...
        {groups.map((group) => (
          ...
          >
            <View className="bg-emerald-50 px-4 py-3">
              ...
              <Text className="mt-1 text-xs text-emerald-700">
                ...
                {group.items.reduce((sum, item) => sum + item.viewCount, 0)}{" "}
                opens
              </Text>
            </View>
            {group.items.map((item) => (
              <Pressable
                key={item.id}
                onPress={() =>
                  router.push(`/(lecturer)/courses/materials/${item.id}` as any)
                }
