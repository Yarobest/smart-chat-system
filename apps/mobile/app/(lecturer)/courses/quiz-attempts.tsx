...
export default function QuizAttemptsScreen(){
  const{quizId}=useLocalSearchParams<{quizId:string}>(); const[data,setData]=useState<any>(null);const[releasing,setReleasing]=useState(false);
  ...
  if(!data)return <SafeAreaView className="flex-1 bg-[#F5F7FA]"><PageLoader label="Loading attempts..."/></SafeAreaView>;
  const totalMarks=data.questions.reduce((sum:number,q:any)=>sum+q.marks,0);
  const release=async()=>{if(releasing)return;try{setReleasing(true);await quizService.release(data.id);Alert.alert('Results released','Students can now see their scores.');load()}catch(e){Alert.alert('Cannot release results',e instanceof Error?e.message:'Please try again.')}finally{setReleasing(false)}};
  return <SafeAreaView className="flex-1 bg-[#051839]"><StatusBar style="light" backgroundColor="#051839"/><ScreenHeader title="Quiz Attempts" fallbackRoute={`/(lecturer)/courses/quizzes/${quizId}`}/><ScrollView className="flex-1 bg-[#F5F7FA]" contentContainerStyle={{padding:16,gap:12,paddingBottom:30}}>
    <View className="rounded-2xl bg-[#0B2A59] p-4"><Text className="text-lg font-extrabold text-white">{data.title}</Text><Text className="mt-1 text-blue-100">{data.attempts.length} attempt{data.attempts.length===1?'':'s'} · {totalMarks} marks</Text><Pressable disabled={releasing} onPress={()=>void release()} className={`mt-4 items-center rounded-lg bg-white py-3 ${releasing?'opacity-50':''}`}><Text className="font-bold text-blue-700">{releasing?'Releasing...':'Release All Graded Results'}</Text></Pressable></View>
