...
export default function SubmissionDetailScreen() {
  const { assignmentId, studentId } = useLocalSearchParams<{
    assignmentId: string;
    studentId: string;
  }>();
  ...
  const load = useCallback(() => {
    ...
    assignmentService
      ...
      .then((result) => {
        ...
        const row = result.students.find(
  ...
  const grade = async (submission: AssignmentSubmission) => {
    ...
    try {
      ...
      Alert.alert(
        "Result released",
        "The student can now see the mark and feedback.",
      );
    } catch (error) {
      Alert.alert(
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#051839]">
      ...
      <ScreenHeader
        ...
        fallbackRoute="/(lecturer)/courses/submissions"
      />
      <KeyboardAwareView>
        <ScrollView
        ...
        >
          <View className="rounded-2xl bg-[#0B2A59] p-4">
            ...
            <Text className="mt-1 text-xs text-blue-200">
              {assignment.course.code} · {student.submissions.length} submission
              version{student.submissions.length === 1 ? "" : "s"}
            </Text>
          </View>
          ...
          {student.submissions.map((submission, index) => (
            ...
            >
              <View className="flex-row items-center justify-between gap-3">
                ...
                <Text
                  className={`rounded-full px-3 py-1 text-xs font-extrabold uppercase ${submission.status === "graded" ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}
                >
                  {submission.status}
                </Text>
              </View>
              ...
              {submission.textResponse ? (
                ...
                <Text className="mt-4 text-sm italic text-slate-400">
              ...
              {submission.attachments.length === 0 ? (
                <Text className="mt-3 text-sm italic text-slate-400">
              ...
              {submission.score !== null && submission.score !== undefined ? (
                <View className="mt-4 rounded-xl bg-emerald-50 p-3">
                  <Text className="font-extrabold text-emerald-800">
                    Awarded: {submission.score}/{assignment.totalMarks}
                  </Text>
                  {submission.feedback ? (
              ...
              {gradingId === submission.id ? (
                <View className="mt-4 gap-3 border-t border-slate-100 pt-4">
                  <TextInput
                    ...
                    placeholder={`Score out of ${assignment.totalMarks}`}
                  ...
                  <Pressable
                    ...
                    onPress={() => void grade(submission)}
                    className={`items-center rounded-full py-3 ${saving || !score.trim() ? "bg-slate-300" : "bg-blue-600"}`}
                  >
                    <Text className="font-bold text-white">
                      {saving ? "Saving..." : "Save and Release Grade"}
                    </Text>
                  </Pressable>
                </View>
              ) : null}
