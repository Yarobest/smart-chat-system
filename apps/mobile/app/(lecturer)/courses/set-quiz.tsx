...
export default function SetQuizScreen() {
  ...
  useEffect(() => {
    ...
    const quizRequest = quizId
      ? quizService.detail(quizId).then((quiz) => {
          setCourseId(quiz.course.offeringId);
          setTitle(quiz.title);
          setInstructions(quiz.instructions);
          setStartAt(new Date(quiz.startAt));
          setEndAt(new Date(quiz.endAt));
          ...
          setQuestions(
            quiz.questions.map((q) => ({
              key: q.id,
              type: q.type.toUpperCase() as DraftQuestion["type"],
              text: q.text,
              options: q.options,
      ...
      : Promise.resolve();
    Promise.all([courseRequest, quizRequest]).catch((e) =>
      Alert.alert("Could not load quiz", e.message),
    );
  }, [quizId]);
  ...
  const changeDate = (selected?: Date) => {
    ...
    const target = picker.startsWith("start") ? startAt : endAt;
    const next = new Date(target);
    if (picker.endsWith("Date"))
      next.setFullYear(
    ...
    picker.startsWith("start") ? setStartAt(next) : setEndAt(next);
    if (Platform.OS !== "ios") setPicker(null);
  };

  const save = async (publish: boolean) => {
    if (!courseId || !title.trim() || !instructions.trim())
      return Alert.alert(
        "Missing information",
        "Select a course and complete the quiz details.",
      );
    const invalid = questions.some(
      (q) =>
        !q.text.trim() ||
        q.marks < 1 ||
        (q.type !== "SHORT_ANSWER" && !String(q.correctAnswer).trim()) ||
        (q.type === "MULTIPLE_CHOICE" && q.options?.some((o) => !o.trim())),
    );
    if (invalid)
      return Alert.alert(
        "Incomplete questions",
        "Complete every question, option, correct answer, and mark.",
      );
    try {
      setSaving(true);
      const input = {
      ...
      if (quizId) await quizService.update(quizId, input);
      else
      ...
      Alert.alert(
        isEditing ? "Quiz updated" : publish ? "Quiz published" : "Draft saved",
        isEditing
          ? "Your safe quiz changes have been saved."
          : publish
            ? "Students will see the quiz in Coursework."
            : "The quiz remains hidden from students.",
        [
          {
            text: "Done",
            onPress: () =>
              router.replace(
                (quizId
                  ? `/(lecturer)/courses/quizzes/${quizId}`
                  : "/(lecturer)/tasks") as any,
              ),
          },
        ],
      );
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#051839]">
      ...
      <ScreenHeader
        ...
        fallbackRoute={
          quizId ? `/(lecturer)/courses/quizzes/${quizId}` : "/(lecturer)/tasks"
        }
      />
      <KeyboardAwareView>
        <ScrollView
        ...
        >
          <View>
            <Text className="mb-2 text-xs font-extrabold text-slate-500">
              COURSE
            </Text>
            <Pressable
              onPress={() => {
                if (!isEditing) setCourseOpen((current) => !current);
              }}
              className="flex-row items-center rounded-2xl border border-slate-200 bg-white px-4 py-3"
            >
              <View className="flex-1">
                <Text
                  className={`text-sm font-bold ${selectedCourse ? "text-slate-900" : "text-slate-400"}`}
                >
                  {selectedCourse
                ...
                {selectedCourse ? (
                  <Text className="mt-1 text-xs text-slate-500">
                    ...
                    {selectedCourse.semester.replace("_", " ")}
                    {isEditing ? " · Locked" : ""}
                  </Text>
                ) : null}
              </View>
                ...
                <ScrollView nestedScrollEnabled>
                  {courses.map((course) => (
                    ...
                    >
                      <View className="flex-1">
                        <Text
                          className={`text-sm font-bold ${courseId === course.id ? "text-blue-700" : "text-slate-800"}`}
                        >
          ...
          <View className="flex-row gap-2">
            {(
              [
                ["startDate", "Opens date", startAt.toLocaleDateString()],
                [
                  "startTime",
                  "Time",
                ...
                ["endDate", "Closes date", endAt.toLocaleDateString()],
                [
                  "endTime",
                  "Time",
              ...
              <Pressable
                ...
                className="flex-1 rounded-xl border border-slate-200 bg-white p-2"
              >
          ...
          {picker ? (
            <View className="rounded-xl bg-white p-2">
              <DateTimePicker
                ...
                onChange={(_e, d) => changeDate(d)}
              />
              {Platform.OS === "ios" ? (
                <Pressable onPress={() => setPicker(null)}>
                  <Text className="text-right font-bold text-blue-600">
                    Done
                  </Text>
                </Pressable>
              ) : null}
          ...
          <View className="rounded-xl bg-white px-4">
            {[
              ["Shuffle questions", shuffleQuestions, setShuffleQuestions],
              ["Shuffle answers", shuffleAnswers, setShuffleAnswers],
          ...
          <View className="mt-2 flex-row items-center justify-between">
            ...
            <Text className="font-bold text-blue-700">
              {questions.reduce((s, q) => s + Number(q.marks || 0), 0)} marks
            </Text>
          </View>
            ...
            >
              <View className="flex-row items-center justify-between">
                ...
                {attemptCount === 0 ? (
                  <Pressable
                    onPress={() =>
                      setQuestions((x) => x.filter((y) => y.key !== q.key))
                    }
                  >
              ...
              <View className="my-3 flex-row gap-2">
                {(
                  ...
                  <Pressable
                    ...
                    className={`flex-1 rounded-lg p-2 ${q.type === type ? "bg-blue-600" : "bg-slate-100"}`}
                  >
                    <Text
                      className={`text-center text-[10px] font-bold ${q.type === type ? "text-white" : "text-slate-600"}`}
                    >
                      {type === "MULTIPLE_CHOICE"
                        ? "Choice"
                        : type === "TRUE_FALSE"
              ...
              {q.type === "MULTIPLE_CHOICE"
                ? q.options?.map((option, i) => (
                    <View key={i} className="mt-2 flex-row items-center">
                      <Pressable
                        onPress={() =>
                          updateQuestion(q.key, { correctAnswer: option })
                        }
                        className={`mr-2 h-6 w-6 rounded-full border ${q.correctAnswer === option && option ? "border-blue-600 bg-blue-600" : "border-slate-300"}`}
                      />
                      <TextInput
                        value={option}
                        onChangeText={(text) => {
                ...
                <View className="mt-3 flex-row gap-2">
                  {["True", "False"].map((v) => (
                    <Pressable
                      key={v}
                      onPress={() =>
                        updateQuestion(q.key, { correctAnswer: v })
                      }
                      className={`flex-1 rounded-xl py-3 ${q.correctAnswer === v ? "bg-blue-600" : "bg-slate-100"}`}
                    >
                      <Text
                        className={`text-center font-bold ${q.correctAnswer === v ? "text-white" : "text-slate-600"}`}
                      >
                        {v}
                      </Text>
                    </Pressable>
              ...
              <TextInput
                ...
                onChangeText={(v) =>
                  updateQuestion(q.key, { marks: Number(v) })
                }
                keyboardType="number-pad"
                placeholder="Marks"
          ...
          {attemptCount === 0 ? (
            <Pressable
              onPress={() => setQuestions((x) => [...x, newQuestion()])}
              className="items-center rounded-xl border border-dashed border-blue-300 bg-blue-50 py-3"
            >
              <Text className="font-bold text-blue-700">+ Add Question</Text>
            </Pressable>
          ) : (
            <Text className="text-center text-xs font-semibold text-slate-500">
              Questions are locked because students have started this quiz.
              Timing and instructions remain editable.
            </Text>
          )}
          {isEditing ? (
            <Pressable
            ...
            <View className="flex-row gap-3">
              <Pressable
                disabled={saving}
                onPress={() => void save(false)}
                className="flex-1 items-center rounded-full border border-blue-600 py-3"
              >
