...
export default function NotesScreen() {
  ...
  const groups = Object.values(
    filtered.reduce(
      (
        map: Record<
          string,
          { code: string; name: string; items: CourseMaterial[] }
        >,
        item,
      ) => {
        map[item.course.offeringId] ??= {
          code: item.course.code,
          name: item.course.name,
          items: [],
        };
        map[item.course.offeringId].items.push(item);
        return map;
      },
      {},
    ),
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#051839]">
      ...
      <ScreenHeader title="Notes & Slides" fallbackRoute="/(student)/tasks" />
      <View className="bg-white pb-3">
        <FilterRow
          filters={["All", "New", "Opened", "Pinned"]}
          active={status}
          counts={{ New: items.filter((item) => item.isNew).length }}
          onSelect={setStatus}
        />
      </View>
      <ScrollView
      ...
      >
        ...
        {groups.map((group) => (
          <View key={group.items[0].course.offeringId}>
            <Text className="mb-2 font-extrabold text-slate-700">
            ...
            {group.items.map((item) => (
              <Pressable
                ...
                onPress={() =>
                  router.push({
                    pathname: "/(student)/tasks/material-detail",
                ...
                <View className="flex-row">
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="mt-1 text-xs text-slate-500">
                      {item.type.toUpperCase()} · v{item.version} ·{" "}
                  ...
                  <Text className="ml-2 self-center text-xl text-slate-400">
                    ›
                  </Text>
                </View>
              </Pressable>
